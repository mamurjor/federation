        ...
        <div class="table-responsive text-nowrap">
            <table class="table">
                ...
                <tbody class="table-border-bottom-0">
                    ...
                    @foreach ($allwingsnomini as $index => $singlevalue)
                        <tr>
                            <td>
                                <span class="fw-medium ms-2">{{ $index + 1 }}</span>
                            </td>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    @if ($singlevalue->status == 0)
                                        <form action="{{ route('wingsnomini.approve', $singlevalue->id) }}" method="POST">
                                            @csrf
                                            @method('POST')
                                            <button class="btn btn-primary" type="submit"> <a class=""
                                                    href=""></a><i class="fa fa-check me-2"></i>Approve</button>
                                    ...
                                    @else
                                        <form action="{{ route('wingsnomini.declined', $singlevalue->id) }}" method="POST">
                                            @csrf
                                            @method('POST')
                                            <button class="btn btn-primary" type="submit"> <a class=""
                                                    href=""></a><i class="fa fa-ban me-2"></i>Declined</button>
                                    ...
                                    <a class="btn btn-danger mt-1" href="{{ route('wingsnomini.delete', $singlevalue->id) }}"
                                        onclick="return confirm('Are Your Suere')"><i class="fa fa-trash me-2"></i>
                                        Delete</a>
